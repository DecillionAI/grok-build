#!/usr/bin/env python3
"""Deploy the web_search tool creature onto a running Caspar node.

The web_search tool gives a Decillion space internet search + page reading: the
space's agents and people run web/news searches and read pages. One docker
creature serves every space (it is global — not bound to a space and stores no
per-space state), so it is deployed once and any space adds it from its
tool-management page.

The creature (`caspar/tools/web_search`) and the shared tool runtime
...
The search-provider API key(s) are read from this process's environment and
**baked into the creature image**, so they never travel in a signal payload an
agent's prompt could influence, and are never written to the repo. With no key,
the tool still deploys and works via the keyless DuckDuckGo fallback.
...
-----------
    ...
    WEB_SEARCH_TOOL_ENTITY_ID   entity id (default web_search)
    WEB_SEARCH_RUN_ENTITY       1 (default) to start it serving after the deploy
    WEB_SEARCH_VM_RAM_MB / _DISK_GB / _CPUS / _MAX_SECONDS   VM resources
    WEB_SEARCH_REBUILD_TIMEOUT  image build wait, seconds (default 480)

    WEB_SEARCH_PROVIDER, TAVILY_API_KEY, BRAVE_API_KEY, SERPAPI_API_KEY,
    BING_SEARCH_API_KEY, GOOGLE_CSE_KEY, GOOGLE_CSE_CX, WEB_SEARCH_*  → baked in

Output (stdout, machine-readable — the CI greps these):
    WEB_SEARCH_TOOL_PROGRAM_ID=<id>
    WEB_SEARCH_TOOL_CREATURE_ID=<id>
    WEB_SEARCH_TOOL_ENTITY_ID=web_search
    WEB_SEARCH_TOOL_FRONTEND_ENTITY_ID=frontend
    WEB_SEARCH_TOOL_VM_ID=<vmId>     (when the standalone runEntity start succeeds)
"""
...
FRONTEND_SOURCE = TOOLS_DIR / TOOL_ID / "frontend" / "search.js"

# Every name the tool reads from its environment. Baked into the image so an
# agent's prompt can never redirect the provider or read the key.
WEB_SEARCH_ENV_NAMES = (
    "WEB_SEARCH_PROVIDER",
    "TAVILY_API_KEY", "TAVILY_KEY",
    "BRAVE_API_KEY", "BRAVE_SEARCH_API_KEY",
    "SERPAPI_API_KEY", "SERPAPI_KEY", "SERP_API_KEY",
    "BING_SEARCH_API_KEY", "BING_API_KEY", "AZURE_BING_KEY",
    "GOOGLE_CSE_KEY", "GOOGLE_API_KEY", "GOOGLE_CSE_CX", "GOOGLE_CSE_ID",
    "WEB_SEARCH_MAX_RESULTS", "WEB_SEARCH_RESULTS_CAP",
    "WEB_SEARCH_MAX_FETCH_BYTES", "WEB_SEARCH_MAX_TEXT_CHARS",
    "WEB_SEARCH_HTTP_TIMEOUT", "WEB_SEARCH_USER_AGENT",
)

_KEY_ENVS = (
    "TAVILY_API_KEY", "TAVILY_KEY", "BRAVE_API_KEY", "BRAVE_SEARCH_API_KEY",
    "SERPAPI_API_KEY", "SERPAPI_KEY", "SERP_API_KEY",
    "BING_SEARCH_API_KEY", "BING_API_KEY", "AZURE_BING_KEY",
    "GOOGLE_CSE_KEY", "GOOGLE_API_KEY",
)


def bake_env() -> Dict[str, str]:
    return {name: os.environ[name].strip() for name in WEB_SEARCH_ENV_NAMES if os.environ.get(name, "").strip()}
...
def deploy_frontend(client: "CasparSignalingClient", program_id: str) -> bool:
    ...
    print("WEB_SEARCH_TOOL_FRONTEND_ENTITY_ID=" + FRONTEND_ENTITY_ID, flush=True)
    return True
...
def compose_dockerfile(files: Dict[str, str]):
    ...
    baked = bake_env()
    if any(baked.get(k) for k in _KEY_ENVS):
        provider = baked.get("WEB_SEARCH_PROVIDER") or "auto"
        info(f"baking a search-provider key into the image (provider={provider})")
    else:
        warn("no search-provider API key in the environment — the creature will deploy and work "
             "via the keyless DuckDuckGo fallback; set TAVILY_API_KEY / BRAVE_API_KEY / SERPAPI_API_KEY "
             "/ BING_SEARCH_API_KEY / GOOGLE_CSE_KEY(+GOOGLE_CSE_CX) for paid-quality results")
    dockerfile = dockerfile + b"\n" + bake_snippet(baked).encode()
    return stamp_context(dockerfile, files)


def main() -> int:
    entity_id = env_any("WEB_SEARCH_TOOL_ENTITY_ID", default=TOOL_ID)
    ...
    if already_current:
        ...
        wait_for_image(program_id, entity_id, timeout=int(env_any("WEB_SEARCH_REBUILD_TIMEOUT", default="480")),
    ...
    print("WEB_SEARCH_TOOL_PROGRAM_ID=" + program_id, flush=True)
    print("WEB_SEARCH_TOOL_CREATURE_ID=" + creature_id, flush=True)
    print("WEB_SEARCH_TOOL_ENTITY_ID=" + entity_id, flush=True)
    ...
    if truthy(env_any("WEB_SEARCH_RUN_ENTITY", default="1")):
        ram = int(env_any("WEB_SEARCH_VM_RAM_MB", default="512"))
        disk = int(env_any("WEB_SEARCH_VM_DISK_GB", default="2"))
        cpus = int(env_any("WEB_SEARCH_VM_CPUS", default="1"))
        max_seconds = vm_max_seconds("WEB_SEARCH_VM_MAX_SECONDS")
        ...
        try:
            ...
            if vm_id:
                ...
                print("WEB_SEARCH_TOOL_VM_ID=" + vm_id, flush=True)
            else:
        ...
        info("WEB_SEARCH_RUN_ENTITY=0 — skipping the standalone runEntity start")
