...
| `prompt.mjs` | Composes what Grok is given: the platform's universal instruction, the agent's skill as persona, the group-chat preamble and roster, the thread's history with `[From → To]` annotations. |
| `platformInstruction.mjs` | The **universal system instruction** an admin sets once for the whole platform (Decillion admin panel → Settings). Read from the `settings` creature's own document (`Json::CreatureNamespace::settings`, path `universalPrompt`) at **execution time**, once per run and briefly cached, then concatenated by `prompt.mjs` with the agent's own system instruction — so an admin edit reaches every agent's next turn with nothing redeployed. Unset (or an unreadable node) means the prompt is exactly what it was before. |
